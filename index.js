...
(async() => {
  ...
  let packageExec = shell.exec('adb shell pm list packages -f | grep tk.elevenk.proxysetter', { silent: true });

  if (packageExec.code !== 0) {
    throw packageExec.stderr;
  }

  if (!packageExec.stdout || packageExec.stdout.split('\n') == 0) {
